...
**Redo (2026-08-02):** during step 3 co-define, the researcher clarified
that SS120 (CCMP1375) is not part of their study — it was removed from
the experiment list and gene-locus resolution, replacing the first pass
of this step's output. The prior version (14 experiments / 6 strains,
including SS120) is preserved in git history at commit `2da4c3b`.
...
- `02_list_np_experiments.py` — `list_experiments(treatment_type=[...],
  ...
  experiments and all SS120/CCMP1375 experiments excluded (see Surprises
  and Decisions).
- `03_resolve_gene_loci.py` — resolved each of the 54 genes to locus tags
  in the 5 strains that appear in the experiment list (MED4, MIT9301,
...
**12 axenic Prochlorococcus N/P starvation experiments** (5 nitrogen, 7
phosphorus), across 7 publications, spanning 5 strains:
...
**54/54 genes resolved to at least one locus tag** among the 5 relevant
...
- **Nitrogen "treatment_type" is not homogeneous.** Of the Prochlorococcus
  nitrogen experiments, 2 are coculture (already out of scope per step 1)
  and, of the axenic ones, 4 are "growth on alternate N source
  (cyanate/urea/nitrite) vs N-replete" rather than starvation/deprivation
  — a different biological question (can the strain use compound X as
  its N source, not "what happens when N runs out"). This mattered here
  because several nitrogen-annotated genes in the 54-gene list are
  themselves alternate-N-source-utilization genes (`cynA/B/D/S`,
  `ureA–G`, `nirA`), so those 4 experiments looked like the most
  on-target evidence for exactly those genes. Raised with the researcher;
  decided to exclude them and keep nitrogen strictly to the
  starvation/deprivation contrasts, for symmetry with the phosphorus side
  (which has no equivalent complication — all axenic phosphorus
...
- **2026-08-02 (redo)** — SS120 (CCMP1375) excluded entirely at the
  researcher's request (not part of their study). Dropped 1 nitrogen
  experiment (azaserine proxy) and 1 phosphorus experiment (Pi-limitation
  iTRAQ), narrowing the strain set from 6 to 5 (MED4, MIT9301, MIT9312,
  MIT9313, NATL2A). All 54 genes still resolve to >=1 locus in the
  remaining 5 strains.
...
- **Outputs produced** —
  ...
  (221 rows). All run via `uv run python analyses/2026-08-02-np_gene_expression_full_analysis/2_kg_selection/scripts/<script>.py`
...
- **QC gate** — confirmed 54/54 genes resolve to >=1 locus tag in the 5
  relevant strains (0 unresolved, after fixing the PMM7xx zero-padding
  case); confirmed experiment counts (5 N / 7 P, 0 SS120) match the
  researcher-approved scope.
...
- **Advance rationale** — the KG entries this analysis will run against
  ...
  ready to move to step 3 (analysis framing: hypotheses, controls,
  expected outcome).
