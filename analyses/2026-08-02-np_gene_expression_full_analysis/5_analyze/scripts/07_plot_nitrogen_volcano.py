...
differential expression under nitrogen starvation only, colored by each
gene's *original* researcher-assigned N/P annotation (not by what this
analysis found -- the color is the source-spreadsheet label).

Only RNASEQ and PROTEOMICS are shown here -- both report a continuous,
finely-graded padj, which is what a volcano plot's y-axis needs to be
meaningful. MICROARRAY padj in this KG build is coarsely binary
(effectively just 0.01 or 1.0 -- see 5_analyze/notebook.md), so a
volcano-style continuous y-axis would misrepresent it as having
confidence gradation it doesn't have; MICROARRAY gets its own,
differently-shaped figure instead (08_plot_nitrogen_microarray.py).
...
selection arbitrary; |log2FC| ranking is unambiguous. Each panel has
its own N/P legend.
...
MUTED = "#898781"
GRID = "#e1e0d9"
PLATFORM_ORDER = ["RNASEQ", "PROTEOMICS"]


def main() -> None:
    df = pd.read_csv(DATA_DIR / "06_nitrogen_volcano_data.csv")
    df = df[df["omics_type"].isin(PLATFORM_ORDER)]
    ...
    for ax, platform in zip(axes, PLATFORM_ORDER):
        ...
        ax.legend(loc="lower left", frameon=False, fontsize=8, title="")
    ...
    fig.suptitle(
        ...
        f"Dashed line: padj=0.05. {n_zero} padj=0 points (RNASEQ) floored just below the smallest "
        "observed nonzero padj. Microarray excluded (binary padj, see 05_nitrogen_microarray.png).",
